...
- component with no props
  - footer with copyright &copy;
    - isolated impl.
    - has embede JS
    - would reasnoably be used in multiple layouts
- component with props
- importance of paticular props
  - children, key
- problem solving
  - fixed element, as prop
  - dynamic component, poly as prop
  - problem with poly-as prop (TSers hate it)
    - altertive is to make fewer unecesarry components
    - what is a necssary component?
- what are necassory componetns
  - hooks
  - calcified/hardened implementation
    - something you easily test in isolation
    -

```jsx
/* something like this could be an important example for React Basics */
function NonPropExample() {
  return <footer>some links {/* new Date.now()*/}<footer>;
}

/* add props to create "new elements" */
/* to get a default impl. take and spread all props */
/* disadvantage is being locked to an element */
...
- demo DOM props
- demo children being included
- demo 'key'
- error for non-DOM props
- leads us into spread position (currently overrides)
- ?? how far do we take it?
  - why you would destructure children
  - Fragments
  - Hooks
  - Why components at all and not functions
  - WTF even a key
...
- brief
- concise
- w/example: showcase ALL props (className, children, id, key)

## Don't like?
- hard-codes element
  - forced into talking about 'as prop' ('div', 'span', 'i')
    - leads into polymorphic as prop (FancyDiv, MyComponent)
      - THIS IS JUST A LOT
      - It's hard to type in TS

## What are we getting?
- a component
  - reusability/isolation/predictability++
  - hooks

## Possible examples
- Button
  - (styled) button
    - disadvantage: forced into a corner styling
  - appyling default default `type="button"` (even tho it's not super rel.)
-->

```jsx
function Example({ children, key }) {
  return <React.Fragment key={key}>{childen}</React.Fragment>;
}
```
...
function Example({ children, key }) {
  ...
  return <React.Fragment key={key}>{t(childen)}</React.Fragment>;
  // problem: I'm dictating what type children are
}
```
...
function Example(children) {
  ...
  // but if not a hook, it should just be a function
  return <>{children}</>;
}

return <div>{myFunctionTransformater("whatever children I had")}</div>;
```
