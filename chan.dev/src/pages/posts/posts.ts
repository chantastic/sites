...
export function path(...segments: string[]) {
  return COLLECTION.constructPathFromSegments(COLLECTION_NAME, ...segments);
}
...
export async function getCollection(
  ...
  sort = (a: CollectionEntry, b: CollectionEntry) =>
    COLLECTION.compareByDate(a.data.publishDate, b.data.publishDate)
) {
  const result = await ASTRO_CONTENT.getCollection(COLLECTION_NAME, filter);
